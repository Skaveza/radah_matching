...
// CORS must happen before bootstrap — bootstrap may output content
...
// Handle preflight before bootstrap runs
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(204);
    exit();
}

// Bootstrap and everything else AFTER headers are sent
...
$routes = [
    ...
    // Admin
    ['GET', '#^/api/admin/purchased_teams_list$#',
        fn() => require __DIR__ . '/../api/admin/purchased_teams_list.php'],

    // Projects
    ['POST', '#^/api/projects$#',
        fn() => require __DIR__ . '/../api/projects/project_create.php'],
    ['GET',  '#^/api/projects$#',
        fn() => require __DIR__ . '/../api/projects/projects_get.php'],
    // PUT must come before the recommendation route so /projects/{id} matches correctly
    ['PUT',  '#^/api/projects/([^/]+)$#',
        fn($id) => require __DIR__ . '/../api/projects/project_update.php'],
    ['GET',  '#^/api/projects/([^/]+)/recommendation$#',
        fn($id) => require __DIR__ . '/../api/projects/project_recommendation_get.php'],

    // Project Sub-Resources (team-roles, milestones, candidates, etc.)
    // IMPORTANT: /bulk must be listed before the generic /{id} pattern
    ['GET',    '#^/api/project-data/([^/]+)$#',
        fn($resource) => require __DIR__ . '/../api/project-data/index.php'],
    ['POST',   '#^/api/project-data/([^/]+)/bulk$#',
        fn($resource) => require __DIR__ . '/../api/project-data/index.php'],
    ['POST',   '#^/api/project-data/([^/]+)$#',
        fn($resource) => require __DIR__ . '/../api/project-data/index.php'],
    ['PUT',    '#^/api/project-data/([^/]+)/([^/]+)$#',
        fn($resource, $id) => require __DIR__ . '/../api/project-data/index.php'],
    ['DELETE', '#^/api/project-data/([^/]+)/([^/]+)$#',
        fn($resource, $id) => require __DIR__ . '/../api/project-data/index.php'],

    // AI
    ['POST', '#^/api/ai/invoke$#',
        fn() => require __DIR__ . '/../api/ai/invoke.php'],
